...
func TestRightsTransfer(t *testing.T) {
	...
	WithDB(t, func(t *testing.T, db *gorm.DB) {
		...
		s.createEntity(ctx, &Membership{
			...
			EntityID:   gtw.ID,
			EntityType: "gateway",
			Rights:     Rights{Rights: []ttnpb.Right{ttnpb.RIGHT_GATEWAY_ALL}},
		})
		...
		store.TransferEntityRights(ctx, usr.Account.OrganizationOrUserIdentifiers(), usr2.Account.OrganizationOrUserIdentifiers(), singleOwnedEntities1)
		singleOwnedEntities1, err = store.FindSingleOwnerMemberships(ctx, usr.Account.OrganizationOrUserIdentifiers())
		if a.So(err, should.BeNil) {
			a.So(singleOwnedEntities1, should.HaveLength, 0)
		}

		rights, err := store.GetMember(ctx, usr2.Account.OrganizationOrUserIdentifiers(), &ttnpb.ApplicationIdentifiers{ApplicationID: app.ApplicationID})
		if a.So(err, should.BeNil) {
			a.So(rights.Rights, should.Resemble, []ttnpb.Right{ttnpb.RIGHT_ALL})
		}

		rights, err = store.GetMember(ctx, usr2.Account.OrganizationOrUserIdentifiers(), &ttnpb.OrganizationIdentifiers{OrganizationID: org.Account.UID})
		if a.So(err, should.BeNil) {
			a.So(rights.Rights, should.Resemble, []ttnpb.Right{ttnpb.RIGHT_ORGANIZATION_ALL})
		}
	})
}
