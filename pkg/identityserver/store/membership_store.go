...
func (s *membershipStore) FindSingleOwnerMemberships(ctx context.Context, id *ttnpb.OrganizationOrUserIdentifiers) ([]ttnpb.Identifiers, error) {
	...
	userMembershipQuery := s.query(ctx, &Membership{}).
		...
		Where(`"memberships"."account_id" = (?)`, accountQuery).
		Where(`"memberships"."rights" = (?) OR "memberships"."rights" = (?) OR "memberships"."rights" = (?) OR "memberships"."rights" = (?)`,
	...
	for i, identifiers := range res {
		query := s.query(ctx, modelForEntityType(identifiers.EntityType))
		switch identifiers.EntityType {
		case "organization":
			query = query.
				Joins(`JOIN "accounts" ON "accounts"."account_type" = 'organization' AND "accounts"."account_id" = "organizations"."id"`).
				Where(`"accounts"."account_type" = ? AND "accounts"."account_id" = ?`, identifiers.EntityType, identifiers.EntityID).
				Select(`"accounts"."uid" AS "friendly_id"`)
		default:
			query = query.
				Where(fmt.Sprintf(`"%[1]ss"."id" = ?`, identifiers.EntityType), identifiers.EntityID).
				Select(fmt.Sprintf(`"%[1]ss"."%[1]s_id" AS "friendly_id"`, identifiers.EntityType))
		}
		var res struct {
			FriendlyID string
		}
		...
		entityIdentifiers[i] = buildIdentifiers(identifiers.EntityType, res.FriendlyID)
	}

	return entityIdentifiers, nil
}

func (s *membershipStore) TransferEntityRights(ctx context.Context, id *ttnpb.OrganizationOrUserIdentifiers, receiverID *ttnpb.OrganizationOrUserIdentifiers, entityList []ttnpb.Identifiers) error {
	defer trace.StartRegion(ctx, "transfer entity rights").End()
	for _, entityID := range entityList {
		userRights, err := s.GetMember(ctx, id, entityID)
		if err != nil {
			return err
		}
		err = s.SetMember(ctx, receiverID, entityID, userRights)
		if err != nil {
			return err
		}
	}
