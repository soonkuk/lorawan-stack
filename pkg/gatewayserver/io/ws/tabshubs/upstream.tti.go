...
// UplinkDataFrame is the LoRaWAN Uplink message of the Tabs Hubs protocol.
...
func (conf TxConfirmation) ToTxAck(ctx context.Context, tokens io.DownlinkTokens, receivedAt time.Time) *ttnpb.TxAcknowledgment {
	...
	if cids, _, ok := tokens.Get(uint16(conf.SeqNo), receivedAt); ok {
		txAck.CorrelationIDs = cids
		txAck.Result = ttnpb.TxAcknowledgment_SUCCESS
	} else {
		logger := log.FromContext(ctx)
		logger.WithField("diid", conf.SeqNo).Debug("Tx acknowledgement either does not correspond to a downlink message or arrived too late")
	}
	return &txAck
}

// HandleUp implements Formatter.
func (f *tabsHubs) HandleUp(ctx context.Context, raw []byte, ids ttnpb.GatewayIdentifiers, conn *io.Connection, receivedAt time.Time) ([]byte, error) {
	logger := log.FromContext(ctx)
	typ, err := Type(raw)
	if err != nil {
